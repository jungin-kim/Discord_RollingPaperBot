...
### 🛡️ 관리자 및 시스템 기능
- **전체 발송**: `/관리자 전체쓰기`로 서버의 모든 멤버(본인 제외)에게 동시에 메시지를 보낼 수 있습니다. (이벤트 공지용)
- **로그 확인**: `/관리자 로그`로 누가 누구에게 보냈는지 전체 기록을 파일(.txt)로 다운로드하여 확인할 수 있습니다. (관리자 전용)
- **데이터 초기화**: `/관리자 초기화`로 저장된 모든 메시지를 즉시 삭제합니다.
- **주기적 자동 초기화**: `/관리자 자동초기화` 명령어로 매달 1일 데이터베이스를 자동으로 비울지 설정합니다 (On/Off 토글).
